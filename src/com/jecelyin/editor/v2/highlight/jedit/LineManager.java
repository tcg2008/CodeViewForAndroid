...
public class LineManager
{
	...
	{
		/*
		 * Performance optimization: assumption is that this method is called
		 * many times for the same line. Not optimizing the first line is
		 * intentional, profiling proves this is the right approach (#3528212).
		 */
	...
	public void contentInserted(int startLine, int offset, int numLines,
			int length, IntegerArray endOffsets)
	{
		...
		if (numLines > 0)
		{
			...
			{
				TokenMarker.LineContext[] lineContextN = new TokenMarker.LineContext[(lineCount
						+ 1) * 2];
			...
			System.arraycopy(this.endOffsets, startLine, this.endOffsets,
					endLine, lineCount - endLine);
			System.arraycopy(foldLevels, startLine, foldLevels, endLine,
					lineCount - endLine);
	...
	public void contentRemoved(int startLine, int offset, int numLines,
			int length)
	{
		int endLine = startLine + numLines;
		...
		if (numLines > 0)
		{
			...
			System.arraycopy(endOffsets, endLine, endOffsets, startLine,
					lineCount - startLine);
			System.arraycopy(foldLevels, endLine, foldLevels, startLine,
					lineCount - startLine);
			System.arraycopy(lineContext, endLine, lineContext, startLine,
					lineCount - startLine);
		} // }}}
	 ...
	 * need to have gapWidth added to their end offsets.
	 */
	private int gapLine;
	...
	private void moveGap(int newGapLine, int newGapWidth, String method)
	{
		...
		else if (newGapLine == -1)
		{
			if (gapWidth != 0)
			{
				...
				// Log.log(Log.DEBUG, this, method + ": update from " + gapLine
				// + " to " + lineCount + " width " + gapWidth);
		...
		} else if (newGapLine < gapLine)
		{
			if (gapWidth != 0)
			{
				...
				// Log.log(Log.DEBUG,this,method + ": update from " + newGapLine
				// + " to " + gapLine + " width " + gapWidth);
