...
public class Catalog
{
	...
	public static Mode[] modes = new Mode[] {
			...
			new Mode("ApacheConf", "apacheconf.xml",
					"{/etc/apache2/{*.conf,{conf.d,{mods,sites}-{available,enabled}}/*},*httpd*.conf}",
					null),
			...
			new Mode("GCBasic", "gcbasic.xml", "*.{gc,gcb}",
					"{*GCBASIC*,*Great Cow BASIC*}"),
			...
			new Mode("Myghty", "myghty.xml", "{autohandler,dhandler,*.myt}",
					null),
			...
			new Mode("Patch", "patch.xml", "*.{diff,patch}",
					"{# HG changeset patch,diff --git *}"),
			new Mode("Perl", "perl.xml", "*.p{[lmh],od}", "#!/*perl*"),
			new Mode("PHP", "php.xml", "*.{php3,php4,php,phtml,inc}",
					"{<\\?php*,#!/*php*}"),
			...
			new Mode("PowerCenter Parameter File",
					"powercenter_parameter_file.xml", "*.par", null),
			...
			new Mode("Python", "python.xml", "*.{py,pyw,sc,jy}",
					"#!*/*python*"),
			...
			new Mode("Roff", "roff.xml", "*.{1,2,3,4,5,6,7,8,9,me,ms,mom,tmac}",
					null),
			...
			new Mode("ShellScript", "shellscript.xml",
					"*.{csh,sh,bash,login,profile,bashrc,bash_profile}",
					"#!/*sh*"),
			new Mode("SHTML", "shtml.xml", "*.{shtml,shtm,ssi}", null),
			new Mode("Slate", "slate.xml", "*.slate", null),
			new Mode("Slax", "slax.xml", "*.slax", null),
			new Mode("Smalltalk", "smalltalk.xml", "*.{st,sources,changes}",
					null),
			...
			new Mode("Template-toolkit", "tthtml.xml", "*.tt{html,css,js}",
					null),
			...
			new Mode("Xml", "xml.xml", "*.{xml,xhtml,xsd,qrc,ui,docbook}",
					"<\\?xml*"),
