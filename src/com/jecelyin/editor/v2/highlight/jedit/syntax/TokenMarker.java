...
/**
 * A token marker splits lines of text into tokens. Each token carries a length
 * field and an identification tag that can be mapped to a color or font style
...
public class TokenMarker
{
	 ...
	 *
	 * @param prevContext
	 *            the context of the previous line, it can be null
	 * @param tokenHandler
	 *            the token handler
	 * @param line
			...
			TokenHandler tokenHandler, Segment line)
	{
		...
		// Logger.d("keywords:"+Arrays.toString(keywords.getKeywords()));
		...
		{
			// {{{ check if we have to stop parsing (happens if the
			// terminateChar has been exceeded)
			if (terminateChar >= 0 && pos - line.offset >= terminateChar
					&& !terminated)
			{
				terminated = true;
				context = new LineContext(ParserRuleSet.getStandardRuleSet(
			...
			if (Character.isWhitespace(ch))
			{
				...
				if (lastOffset != pos)
				{
					tokenHandler.handleToken(line, context.rules.getDefault(),
							lastOffset - line.offset, pos - lastOffset,
							context);
				}
				...
				if (keywords != null || context.rules.getRuleCount() != 0)
				{
							...
							&& noWordSep.indexOf(ch) == -1)
					{
						...
						tokenHandler.handleToken(line,
								...
								lastOffset - line.offset, 1, context);
						lastOffset = pos + 1;
					}
				}
		...
		tokenHandler.handleToken(line, Token.END, pos - line.offset, 0,
				context);
	...
	// {{{ Instance variables
	private final Map<String, ParserRuleSet> ruleSets = new Hashtable<String, ParserRuleSet>(
			64);
	...
	private boolean offsetMatches(int offset, int posMatch)
	{
		if ((posMatch & ParserRule.AT_LINE_START) == ParserRule.AT_LINE_START)
		{
			if (offset != line.offset)
			{
				...
				& ParserRule.AT_WHITESPACE_END) == ParserRule.AT_WHITESPACE_END)
		{
			if (offset != whitespaceEnd)
			{
				...
				& ParserRule.AT_WORD_START) == ParserRule.AT_WORD_START)
		{
	...
	/**
	 * Checks if the rule matches the line at the current position as its start
	...
	private boolean handleRuleStart(ParserRule checkRule)
	{
		...
		{
			if (-1 == Arrays.binarySearch(checkRule.upHashChars,
		...
		if ((checkRule.action & ParserRule.REGEXP) == 0)
		{
			...
			if (!SyntaxUtilities.regionMatches(context.rules.getIgnoreCase(),
		...
		} else
		{
			...
			CharSequence charSeq = new SegmentCharSequence(line,
					pos - line.offset, line.count - (pos - line.offset));
			...
			} else
			{
				...
				/*
				 * workaround for hang if match was zero-width. not sure if
		...
		} else
		{
			...
			markKeyword((checkRule.action
					& ParserRule.MARK_PREVIOUS) != ParserRule.MARK_PREVIOUS);
			...
			case ParserRule.SEQ:
				...
				if ((checkRule.action & ParserRule.REGEXP) != 0)
				{
					handleTokenWithSpaces(tokenHandler, checkRule.token,
							pos - line.offset, matchedChars, context);
				} else
				{
					tokenHandler.handleToken(line, checkRule.token,
							pos - line.offset, matchedChars, context);
				}
				...
				{
					context = new LineContext(checkRule.delegate,
							context.parent);
					keywords = context.rules.getKeywords();
				}
				break;
			...
			case ParserRule.EOL_SPAN:
				...
				if ((checkRule.action & ParserRule.REGEXP) != 0)
				{
					handleTokenWithSpaces(tokenHandler, tokenType,
							pos - line.offset, matchedChars, context);
				} else
				{
					tokenHandler.handleToken(line, tokenType, pos - line.offset,
				...
				/*
				 * substitute result of matching the rule start into the end
				 * string.
				 *
				 * eg, in shell script mode, <<\s*(\w+) is matched into \<$1\>
				 * to construct rules for highlighting read-ins like this <<EOF
				 * ... EOF
				 */
				if (match != null && match.groupCount() > 0)
				{
					...
					{
						char[] pattern = checkRule.endRegexp.pattern()
								.toCharArray();
						pattern = substitute(match, pattern, true);

						spanEndSubstRegex = Pattern
								.compile(new String(pattern));
					}
				}
	...
	/**
	 * Checks if the rule matches the line at the current position as its end
	 * and handles the rule if it does match
	 */
	private boolean handleRuleEnd(ParserRule checkRule)
	{
		...
		if ((checkRule.action & ParserRule.MARK_FOLLOWING) == 0)
		{
			if ((checkRule.action & ParserRule.END_REGEXP) == 0)
			{
				...
				if (!SyntaxUtilities.regionMatches(
						context.rules.getIgnoreCase(), line, pos,
						pattern.array))
				{
					return false;
				}
		...
		if ((context.inRule.action & ParserRule.MARK_FOLLOWING) != 0)
		{
			if (pos != lastOffset)
			{
				tokenHandler.handleToken(line, context.inRule.token,
						lastOffset - line.offset, pos - lastOffset, context);
			}
	...
	private void handleNoWordBreak()
	{
		if (context.parent != null)
		{
					...
					& ParserRule.NO_WORD_BREAK) != 0)
			{
				if (pos != lastOffset)
				{
					tokenHandler.handleToken(line, rule.token,
							lastOffset - line.offset, pos - lastOffset,
							context);
				}
			...
			byte tokenType, int start, int len, LineContext context)
	{
		...
		for (int i = start; i < end; i++)
		{
			if (Character.isWhitespace(line.array[i + line.offset]))
			{
				if (last != i)
				{
					tokenHandler.handleToken(line, tokenType, last, i - last,
							context);
				}
		...
		if (last != end)
		{
			tokenHandler.handleToken(line, tokenType, last, end - last,
					context);
		}
	} // }}}
	...
	private void markKeyword(boolean addRemaining)
	{
		...
		if (keywords != null)
		{
			...
			if (id != Token.NULL)
			{
				tokenHandler.handleToken(line, id, lastOffset - line.offset,
						len, context);
				lastOffset = pos;
				return;
			}
	...
	/**
	 * Perform substitution references in <code>end</code> to the matched groups
	 * in <code>match</code>. In particular, "$1" is replaced with the first
	 * match group in <code>match</code>, $2 is replaced with the second, all
	 * the way up to "$9". Moreover, if group <code>i</code> matched a single
	 * bracket character, then "~i" is replaced with the complementary bracket.
	 *
	 * @param match
	 *            the <code>Matcher</code> produced by matching the "start"
	 *            regex
	 * @param end
	 *            the pattern to substitute in to
	 * @param escape
	 *            if true, then escape matched text before inserting into
	 *            <code>end</code>, so that the result can be interpreted as a
	 *            regex pattern.
			...
			if (ch == '$' || ch == '~')
			{
				...
				else
				{
					...
					} else
					{
						...
						if (s.length() == 1)
						{
							char b = TextUtilities
									.getComplementaryBracket(s.charAt(0), null);
							if (b == '\0')
								b = s.charAt(0);
							buf.append(b);
						} else
	...
	private boolean checkHashString(ParserRule rule)
	{
		...
		{
			if (Character
					.toUpperCase(line.array[pos + i]) != rule.upHashChar[i])
			{
				return false;
			}
		}
	...
	public static class LineContext
	{
		...
		public LineContext(ParserRuleSet rs, LineContext lc)
		{
			...
			/*
			 * SPANs with no delegate need to propagate the escape rule to the
			 * child context, so this is needed.
			 */
		...
		public int hashCode()
		{
			...
			code += (spanEndSubstRegex != null) ? spanEndSubstRegex.hashCode()
					: 0;
			return code;
		} // }}}
		...
		public boolean equals(Object obj)
		{
			if (obj instanceof LineContext)
			{
				...
				return lc.inRule == inRule && lc.rules == rules
						...
						&& Objects.equals(spanEndSubstRegex,
								lc.spanEndSubstRegex);
			} else
				return false;
		} // }}}
		...
		/**
		 * Sets the current rule being processed and adjusts the escape rule for
		 * the context based on the rule.
		 */
