...
public abstract class XModeHandler extends DefaultHandler
{
	...
	{
		return XMLUtilities.findEntity(systemId, "xmode.dtd",
				XModeHandler.class);
	} // }}}
			...
			Attributes attrs)
	{
		...
		} else if (qName.equals("RULES"))
		{
			...
			if (tag.lastDigitRE != null)
			{
				try
				{
					rules.setDigitRegexp(Pattern.compile(tag.lastDigitRE,
							tag.lastIgnoreCase ? Pattern.CASE_INSENSITIVE : 0));
			...
			if (tag.lastEscape != null)
				rules.setEscapeRule(
						ParserRule.createEscapeRule(tag.lastEscape));
	...
	public void endElement(String uri, String localName, String name)
	{
		...
		{
			if (tag.lastDelegateSet != null && !tag.tagName.equals("IMPORT")
					&& !tag.lastDelegateSet.getModeName().equals(modeName))
			{
				Mode mode = ModeProvider.instance
						.getMode(tag.lastDelegateSet.getModeName());
			...
			else if (tag.tagName.equals("SPAN"))
			{
				...
				rules.addRule(ParserRule.createSpanRule(tag.lastStartPosMatch,
						tag.lastStart.toString(), tag.lastEndPosMatch,
						tag.lastEnd.toString(), tag.lastDelegateSet,
						tag.lastTokenID, tag.lastMatchType, tag.lastNoLineBreak,
			...
			else if (tag.tagName.equals("SPAN_REGEXP"))
			{
				...
				try
				{
					if (null != tag.lastHashChars)
					{
						rules.addRule(ParserRule.createRegexpSpanRule(
								tag.lastStartPosMatch,
								tag.lastHashChars.toCharArray(),
								tag.lastStart.toString(), tag.lastEndPosMatch,
								tag.lastEnd.toString(), tag.lastDelegateSet,
			...
			else if (tag.tagName.equals("EOL_SPAN"))
			{
				...
				rules.addRule(
						ParserRule.createEOLSpanRule(tag.lastStartPosMatch,
								tag.lastStart.toString(), tag.lastDelegateSet,
								tag.lastTokenID, tag.lastMatchType));
			} // }}}
				// {{{ EOL_SPAN_REGEXP
			...
			else if (tag.tagName.equals("MARK_FOLLOWING"))
			{
				...
				rules.addRule(ParserRule.createMarkFollowingRule(
						tag.lastStartPosMatch, tag.lastStart.toString(),
						tag.lastTokenID, tag.lastMatchType));
			} // }}}
				// {{{ MARK_PREVIOUS
			else if (tag.tagName.equals("MARK_PREVIOUS"))
			{
				...
				rules.addRule(ParserRule.createMarkPreviousRule(
						tag.lastStartPosMatch, tag.lastStart.toString(),
						tag.lastTokenID, tag.lastMatchType));
			} // }}}
				// {{{ Keywords
			else if (!tag.tagName.equals("END") && !tag.tagName.equals("BEGIN")
	...
	/**
	 * Reports an error. You must override this method so that the mode loader
	 * can do error reporting.
	 *
	 * @param msg
	 *            The error type
	 * @param subst
	 *            A <code>String</code> or a <code>Throwable</code> containing
	...
	/**
	 * Returns the token marker for the given mode. You must override this
	 * method so that the mode loader can resolve delegate targets.
	 *
	 * @param mode
	 *            The mode name
	 * @since jEdit 4.2pre1
	...
	/**
	 * Finds the first element whose tag matches 'tagName', searching backwards
	 * in the stack.
	 */
	...
	/**
	 * Hold info about what tag was read and what attributes were set in the XML
	 * file, to be kept by the handler in a stack (similar to the way tag names
	 * were kept in a stack before).
	 */
	private class TagDecl
	{
		...
		public TagDecl(String tagName, Attributes attrs)
		{
			...
			lastAtWhitespaceEnd = "TRUE"
					.equals(attrs.getValue("AT_WHITESPACE_END"));
			...
			lastHighlightDigits = "TRUE"
					.equals(attrs.getValue("HIGHLIGHT_DIGITS"));
			...
			if (tmp != null)
			{
				...
				else
				{
					...
					if (delegateMarker == marker && lastDelegateSet == null)
					{
						...
						lastDelegateSet = new ParserRuleSet(delegateMode,
								delegateSetName);
		...
		{
			if (tagName.equals("EOL_SPAN") || tagName.equals("EOL_SPAN_REGEXP")
					|| tagName.equals("MARK_PREVIOUS")
					|| tagName.equals("MARK_FOLLOWING") || tagName.equals("SEQ")
					|| tagName.equals("SEQ_REGEXP") || tagName.equals("BEGIN"))
			{
				TagDecl target = this;
				...
				if (target.lastStart == null)
				{
					...
					target.lastStartPosMatch = ((target.lastAtLineStart
							? ParserRule.AT_LINE_START : 0)
							| (target.lastAtWhitespaceEnd
									? ParserRule.AT_WHITESPACE_END : 0)
							| (target.lastAtWordStart ? ParserRule.AT_WORD_START
									: 0));
			...
			} else if (tagName.equals("END"))
			{
				...
				if (target.lastEnd == null)
				{
					...
					target.lastEndPosMatch = ((this.lastAtLineStart
							? ParserRule.AT_LINE_START : 0)
							| (this.lastAtWhitespaceEnd
									? ParserRule.AT_WHITESPACE_END : 0)
							| (this.lastAtWordStart ? ParserRule.AT_WORD_START
									: 0));
