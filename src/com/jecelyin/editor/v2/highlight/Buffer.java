...
public class Buffer
{
	...
	/**
	 * Returns the specified line in a <code>Segment</code>.
	 * <p>
	 * <p/>
	 * Using a <classname>Segment</classname> is generally more efficient than
	 * using a <classname>String</classname> because it results in less memory
	 * allocation and array copying.
	 * <p>
	 * <p/>
	...
	/**
	 * Returns the specified line from the starting point passed in
	 * relativeStartOffset in a <code>Segment</code>.
	 * <p>
	 * <p/>
	 * Using a <classname>Segment</classname> is generally more efficient than
	 * using a <classname>String</classname> because it results in less memory
	...
	public void getLineText(int line, int relativeStartOffset, Segment segment)
	{
		...
		if ((start + relativeStartOffset) > end)
		{
			throw new IllegalArgumentException(
					"This index is outside the line length (start+relativeOffset):"
							+ start + " + " + relativeStartOffset + " > "
							+ "endffset:" + end);
		} else
		{
			getText(start + relativeStartOffset,
					end - start - relativeStartOffset - 1, segment);
		}
	...
	{
		if (start < 0 || length < 0 || start + length > editable.length())
	...
	/**
	 * Returns the specified text range in a <code>Segment</code>.
	 * <p>
	 * <p/>
	 * Using a <classname>Segment</classname> is generally more efficient than
	 * using a <classname>String</classname> because it results in less memory
	...
	protected TokenMarker.LineContext markTokens(Segment seg,
			TokenMarker.LineContext prevContext, TokenHandler _tokenHandler)
	{
		TokenMarker.LineContext context;
		// if (longBufferMode && longLineLimit != 0 && longLineLimit <
		// seg.length())
		// {
	 ...
	 *            The line number
	 * @param tokenHandler
	 *            The token handler that will receive the syntax tokens
	 * @since jEdit 4.1pre1
	 */
	public void markTokens(int lineIndex, TokenHandler tokenHandler)
	{
		...
		if (lineIndex < 0 || lineIndex >= lineMgr.getLineCount())
			throw new ArrayIndexOutOfBoundsException("lineCount="
					+ lineMgr.getLineCount() + "; index=" + lineIndex);
		...
		// Log.log(Log.DEBUG, this, "tokenize from " + start + " to " +
		// lineIndex);
		...
		for (int i = start; i <= lineIndex; i++)
		{
			...
			TokenHandler _tokenHandler = i == lineIndex ? tokenHandler
					: DummyTokenHandler.INSTANCE;
		...
		else if (firstInvalidLineContext == -1)/* do nothing */ ;
		else
		{
			lineMgr.setFirstInvalidLineContext(
					Math.max(firstInvalidLineContext, lineIndex + 1));
		}
	} // }}}
	...
	public void setEditable(Editable editable)
	{
		...
		if (!(editable instanceof SpannableStringBuilder))
			throw new RuntimeException("Can't set a "
					+ editable.getClass().getName() + " to Buffer");
